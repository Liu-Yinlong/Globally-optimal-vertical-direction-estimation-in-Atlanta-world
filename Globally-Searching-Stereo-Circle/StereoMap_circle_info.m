function [StereoMap_direction_circle,iter,L,U,branches] = StereoMap_circle_info(data,epsilon)
%OUR_DIRECTION_SEARCH_CIRCLE 此处显示有关此函数的摘要
%   此处显示详细说明
B=[0;0;1];
branches=[];
...
% figure;
% title('Evolution of the bounds of stereo (circle)')
% hold on
% 
...
while(1)
    ...
    for i=1:4
        [new_lower(i),new_upper(i)] = StereoMap_circle_bounds(c_solid(:,i),new_half_side,data,epsilon);
    end
...
%     drawnow
    ...
    if(E_upper==E_lower) 
        ...
        StereoMap_direction_circle = StereoMap_point_2d_3d(StereoMap_direction_2d(1),StereoMap_direction_2d(2));
